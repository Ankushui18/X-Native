...
fn clip_once(s: &[Pt], c: &[Pt], op: ClipOp) -> Result<Vec<Vec<Pt>>, bool> {
    ...
    if xpts.len() % 2 != 0 { return Err(true); } // grazing contact
    ...
    let mut s_head = usize::MAX;
    let mut build = |pts: &[Pt], on: &mut Vec<Vec<(f64, usize)>>, id2: &mut Vec<usize>, arena: &mut Vec<V>| -> usize {
        let start = arena.len();
        ...
        let end = arena.len();
        for k in start..end {
            arena[k].next = if k + 1 < end { k + 1 } else { start };
            arena[k].prev = if k > start { k - 1 } else { end - 1 };
        }
        start
    };
    s_head = build(s, &mut on_s, &mut id2s, &mut arena);
