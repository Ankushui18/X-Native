...
fn fingerprint(c: &RenderCommand) -> String {
    match c {
        ...
        RenderCommand::Glyphs {
            ...
            "g{:?}{text}{size}{max_width}{font:?}{brush:?}{runs:?}",
            transform.as_coeffs()
        ),
        RenderCommand::Image {
            transform,
        ...
        for cmd in &tree.commands {
            match cmd {
                ...
                RenderCommand::StrokePath {
                    ...
                    let mut stroke = vello::kurbo::Stroke::new(*width)
                        .with_caps(cap)
                        .with_join(join)
                        .with_miter_limit(options.miter_limit);
                    if !options.dash.is_empty() {
                        stroke =
                            stroke.with_dashes(options.dash_offset, options.dash.iter().copied());
                    }
                ...
                RenderCommand::Glyphs {
                    ...
                    let drew = if let Some(fm) = self.fonts {
                        if !runs.is_empty() {
                            let key = x_text::TextLayoutKey::new_rich(
                                runs,
                                *size,
                                *max_width,
                                font.as_deref(),
                                ...
                                x_text::ShapedTextCache::global().get_or_shape(fm, key)
                            {
                                for g in block.glyphs.iter() {
                                    let solid;
                                    let b: &Brush = if g.color.components[3] == 0.0 {
                                        brush
                                    } else {
                                        solid = Brush::Solid(g.color);
                                        &solid
                                    };
                                    scene.fill(
                                        Fill::NonZero,
                            ...
                            let key = x_text::TextLayoutKey::new_styled(
                                text,
                                *size,
                                *max_width,
                                font.as_deref(),
