...
impl Assets {
    ...
    pub fn sync_store(&mut self, store: &x_core::AssetStore) -> usize {
        ...
        for rec in store.iter_sorted() {
            if rec.mime != "image/png" || self.images.contains_key(&rec.id) {
                continue;
            }
            if self.load_png_bytes(&rec.id.clone(), &rec.bytes).is_ok() {
                added += 1;
            }
        }
        added
    ...
    pub fn insert_raw(&mut self, name: &str, img: ImageBrush) {
    ...
    pub fn memory_bytes(&self) -> usize {
        self.images
            .values()
            .map(|i| (i.image.width * i.image.height * 4) as usize)
            .sum()
    }
