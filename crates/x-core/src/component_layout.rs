...
use crate::{
    apply_auto_layout, components::{find_master, typed_overrides, OverrideValue},
    Node, NodeKind, Sizing, Variables,
};
    ...
    fn pass(n: &mut Node, ovr: &std::collections::HashMap<String, OverrideValue>, measure: MeasureFn) {
        if let Some(v) = ovr.get(&n.id) {
            match v {
                ...
                OverrideValue::Fill(c) => n.fill = crate::Paint::Solid(*c),
...
mod tests {
    use super::*;
    use crate::{AutoLayout, Color, LayoutDirection, Node, Variables};
    ...
    fn button_doc() -> Node {
        ...
        let mut inner = Node::frame("btn-root", 0.0, 0.0)
            .auto_layout(AutoLayout {
                ...
                gap: 8.0, padding: 12.0, sizing: crate::Sizing::Hug,
                ..Default::default()
            })
    ...
    fn sync_grows_instance_and_resizes_layout_parent() {
        ...
        let toolbar = Node::frame("toolbar", 0.0, 0.0)
            .auto_layout(AutoLayout {
                direction: LayoutDirection::Horizontal,
                gap: 10.0, padding: 6.0, sizing: crate::Sizing::Hug,
                ..Default::default()
            })
            .child(inst)
    ...
    fn detach_applies_overrides_and_keeps_nested_instances() {
        ...
        assert!(matches!(&bg.fill, crate::Paint::Solid(c) if c.r == 0xff));
