...
fn apply_transform_attr(node: &mut ImportNode, attrs: &[(String, String)]) {
    ...
    if let Some(op) = attr_num(attrs, "opacity") {
        node.opacity = op as f32;
    }
}
...
pub(crate) fn parse_path_d(d: &str) -> Vec<PathCmd> {
    ...
    let flush =
        |cmd: char, nums: &mut Vec<f64>, out: &mut Vec<PathCmd>, cx: &mut f64, cy: &mut f64| {
            ...
            match cmd.to_ascii_uppercase() {
                ...
                'V' => {
                    for v in nums.iter() {
                        let y = if rel { *cy + v } else { *v };
                        out.push(PathCmd::LineTo(*cx, y));
                        *cy = y;
                    }
                }
                'C' => {
                    for six in nums.chunks(6) {
                        if six.len() == 6 {
                            ...
                            out.push(PathCmd::CurveTo(x1, y1, x2, y2, x, y));
                            *cx = x;
                            *cy = y;
                        }
                    }
    ...
    for ch in d.chars() {
        if ch.is_ascii_alphabetic() {
            ...
            if cmd != ' ' {
                flush(cmd, &mut nums, &mut out, &mut cx, &mut cy);
            }
            cmd = ch;
            if ch == 'Z' || ch == 'z' {
                flush(cmd, &mut nums, &mut out, &mut cx, &mut cy);
            ...
            if !num_buf.is_empty() && !num_buf.ends_with('e') {
                push_num(&mut num_buf, &mut nums);
            }
            num_buf.push(ch);
        } else {
...
fn parse_children(lexer: &mut XmlLexer, parent: &mut ImportNode) -> Result<(), String> {
    ...
    loop {
        match lexer.next_tag()? {
            ...
            XmlTag::Close(_) => {
                if let Some(t) = pending_text_node.take() {
            ...
            tag @ (XmlTag::Open(..) | XmlTag::SelfClose(..)) => {
                ...
                match name.as_str() {
                    ...
                    "line" => {
                        ...
                        let stroke_c = attr(&attrs, "stroke")
                            .and_then(parse_hex_color)
                            .unwrap_or(Color::BLACK);
                        let mut n = with_id(ImportNode::new(ImportKind::Line))
                            .at(x1, y1)
                            .size((x2 - x1).abs().max(1.0), 1.0);
                        n.stroke = Some((
                            Paint::Solid(stroke_c),
                            attr_num(&attrs, "stroke-width").unwrap_or(1.0),
                        ));
                        apply_transform_attr(&mut n, &attrs);
                        if !self_closed {
                            skip_element(lexer)?;
                    ...
                    "path" => {
                        ...
                        let mut n = with_id(ImportNode::new(ImportKind::Path { cmds }))
                            .size(w, h)
                            .fill(Paint::Solid(attr_fill(&attrs)));
                        if let (Some(sc), Some(sw)) = (
                            attr(&attrs, "stroke").and_then(parse_hex_color),
                            attr_num(&attrs, "stroke-width"),
                        ) {
                    ...
                    "text" => {
                        ...
                        if self_closed {
                            ...
                            pending_text_node = Some(n);
                        }
                    }
                    "defs" | "style" | "clipPath" | "mask" | "linearGradient"
