...
fn miniz_oxide_stub_compress(data: &[u8]) -> Vec<u8> {
    ...
    for (i, block) in data.chunks(65535).enumerate() {
        let last = if (i + 1) * 65535 >= data.len() { 1u8 } else { 0u8 };
        out.push(last);
...
fn sketch_embedded_bitmap_lands_in_the_store_with_asset_uri() {
    ...
    let NodeKind::Image { asset, .. } = &img.kind else { panic!("not an image: {:?}", img.kind) };
    assert!(asset.starts_with("asset://"), "bitmap ref rewritten to content id: {asset}");
    let rec = doc.assets.get(asset).expect("bytes registered in the store");
...
fn document_with_embedded_assets_is_portable_through_x() {
    ...
    let NodeKind::Image { asset, .. } = &re.pages[0].children[0].kind else { panic!() };
    let rec = re.assets.get(asset).expect("asset resolves after reload");
    assert_eq!(rec.dimensions, Some((6, 4)));
    // byte-stable round trip with assets present
    assert_eq!(save_x(&re), text, "save(load(save)) byte-identical with assets");
    ...
    let has_image_cmd = tree.commands.iter().any(|c| matches!(c, RenderCommand::Image { asset, .. } if asset.starts_with("asset://")));
    assert!(has_image_cmd, "render IR references the asset uri");
    let sink = VelloSink { assets: Some(&cache), fonts: None };
...
fn dedup_same_image_used_twice_stored_once() {
    ...
    let ids: Vec<String> = doc.pages[0].children.iter().filter_map(|c| match &c.kind {
