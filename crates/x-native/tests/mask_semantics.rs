...
fn mask_clips_vector_sibling() {
    let tri = vec![PathCmd::MoveTo(0.0, 80.0), PathCmd::LineTo(40.0, 0.0), PathCmd::LineTo(80.0, 80.0), PathCmd::Close];
    let doc = Node::frame("page", 400.0, 300.0)
...
fn mask_clips_group_subtree() {
    ...
    let fills: Vec<usize> = ks.iter().enumerate().filter(|(_, k)| **k == "fill").map(|(i, _)| i).collect();
    assert_eq!(fills.len(), 2, "both group children painted: {ks:?}");
    for f in fills { assert!(f > ci && f < pi, "group fill {f} outside clip [{ci},{pi}]: {ks:?}"); }
}
...
fn mask_clips_component_instance_with_auto_layout() {
    ...
    let master = Node::component("master", "Chip", 160.0, 40.0)
        .child({
    ...
    let inside_fills = ks.iter().enumerate().filter(|(i, k)| **k == "fill" && *i > ci && *i < pi).count();
    assert!(inside_fills >= 2, "instance content ({inside_fills} fills) must render inside the mask clip: {ks:?}");
...
fn mask_boolean_gradient_effect_chain_survives_export() {
    ...
    assert!(svg.contains("<linearGradient"), "svg carries the gradient def");
