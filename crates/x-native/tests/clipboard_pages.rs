...
fn two_rects() -> Node {
    Node::frame("page-1", 800.0, 600.0)
        .child(Node::rect("r1", 10.0, 10.0, 100.0, 80.0, Color::from_rgb8(0xff, 0, 0)))
        .child(Node::rect("r2", 200.0, 10.0, 100.0, 80.0, Color::from_rgb8(0, 0xff, 0)))
}
...
fn cut_then_paste_round_trips() {
    ...
    ed.cut();
    assert!(x_native::editor::find(&ed.root, "r1").is_none(), "cut removes the node");
    ...
    assert!(x_native::editor::find(&ed.root, "r1").is_some(), "undo restores the cut node");
}
...
fn typography_bindings_change_text_geometry() {
    ...
    let base = x_native::text::node_text_outlines_styled(
        &fm, "Spacing", 24.0, 10_000.0, None, Color::from_rgb8(0, 0, 0), 0.0, 1.2).unwrap();
    let wide = x_native::text::node_text_outlines_styled(
        &fm, "Spacing", 24.0, 10_000.0, None, Color::from_rgb8(0, 0, 0), 4.0, 1.2).unwrap();
    let last_x = |g: &[x_native::text::OutlineGlyph]| g.iter()
    ...
    let tall = x_native::text::node_text_outlines_styled(
        &fm, "a\nb\nc", 24.0, 40.0, None, Color::from_rgb8(0, 0, 0), 0.0, 2.0).unwrap();
    let short = x_native::text::node_text_outlines_styled(
        &fm, "a\nb\nc", 24.0, 40.0, None, Color::from_rgb8(0, 0, 0), 0.0, 1.0).unwrap();
...
fn independent_corner_radii_render_and_persist() {
    ...
    let mut n = Node::rect("btn", 0.0, 0.0, 100.0, 50.0, Color::from_rgb8(0x33, 0x66, 0xff)).radius(8.0);
    ...
    assert!(tree.commands.iter().any(|c| matches!(c, x_native::RenderCommand::FillPath { .. })), "rect fill renders");
    // persistence
    let mut d = Document::new();
    d.pages.push(page);
    let d2 = fileio::DocumentV2 { doc: d, ..Default::default() };
    let text = fileio::save_x_v2(&d2);
...
fn svg_clipboard_roundtrip_stays_editable() {
    ...
    let frame = Node::frame("clip", 200.0, 100.0)
        .child(Node::rect("r1", 10.0, 10.0, 80.0, 40.0, Color::from_rgb8(0x22, 0xc5, 0x5e)).radius(6.0));
    let vars = x_native::Variables::default();
