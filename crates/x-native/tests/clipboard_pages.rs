...
use arco_native::{Color, Document, Node};

fn two_rects() -> Node {
    Node::frame("page-1", 800.0, 600.0)
        .child(Node::rect("r1", 10.0, 10.0, 100.0, 80.0, Color::rgb8(0xff, 0, 0)))
        .child(Node::rect("r2", 200.0, 10.0, 100.0, 80.0, Color::rgb8(0, 0xff, 0)))
}
...
fn copy_paste_multiselect_preserves_properties_and_editability() {
    ...
    for id in &ids {
        let n = arco_native::editor::find(&ed.root, id).expect("pasted node exists");
        assert!(n.transform.x > 10.0, "pasted with offset");
    }
    ed.selection = ids.clone();
    ed.move_selection(5.0, 0.0);
    let n = arco_native::editor::find(&ed.root, &ids[0]).unwrap();
    assert!((n.transform.x - 31.0).abs() < 1e-6, "pasted node moved (editable)");
}
...
fn cut_then_paste_round_trips() {
    ...
    ed.cut();
    assert!(arco_native::editor::find(&ed.root, "r1").is_none(), "cut removes the node");
    ...
    assert!(arco_native::editor::find(&ed.root, "r1").is_some(), "undo restores the cut node");
}
...
fn page_rename_survives_save_and_reload() {
    ...
    assert_eq!(loaded.metadata.name, "Brand Dashboard", "file display name persists");
}
...
fn typography_bindings_change_text_geometry() {
    ...
    if fm.load_system_fonts() == 0 { return; } // headless CI without fonts
    let base = arco_native::text::node_text_outlines_styled(
        &fm, "Spacing", 24.0, 10_000.0, None, Color::rgb8(0, 0, 0), 0.0, 1.2).unwrap();
    let wide = arco_native::text::node_text_outlines_styled(
        &fm, "Spacing", 24.0, 10_000.0, None, Color::rgb8(0, 0, 0), 4.0, 1.2).unwrap();
    let last_x = |g: &[arco_native::text::OutlineGlyph]| g.iter()
        .map(|o| o.transform.as_coeffs()[4]).fold(f64::MIN, f64::max);
    assert!(last_x(&wide.0) > last_x(&base.0) + 10.0, "letter spacing widens layout");
    let tall = arco_native::text::node_text_outlines_styled(
        &fm, "a\nb\nc", 24.0, 40.0, None, Color::rgb8(0, 0, 0), 0.0, 2.0).unwrap();
    let short = arco_native::text::node_text_outlines_styled(
        &fm, "a\nb\nc", 24.0, 40.0, None, Color::rgb8(0, 0, 0), 0.0, 1.0).unwrap();
...
fn independent_corner_radii_render_and_persist() {
    ...
    let mut n = Node::rect("btn", 0.0, 0.0, 100.0, 50.0, Color::rgb8(0x33, 0x66, 0xff)).radius(8.0);
    n.corner_radii = Some([2.0, 8.0, 20.0, 8.0]);
    let page = Node::frame("p", 200.0, 100.0).child(n);
    let vars = arco_native::Variables::default();
    let tree = arco_native::build_render_tree(&page, &vars);
    // a strokeless rect emits exactly one FillPath (the transparent
    // frame emits none) — assert the fill is there
    assert!(tree.commands.iter().any(|c| matches!(c, arco_native::RenderCommand::FillPath { .. })), "rect fill renders");
    // persistence
    let mut d = Document::new();
    d.pages.push(page);
    let mut d2 = fileio::DocumentV2::default();
    ...
    let btn = arco_native::editor::find(&loaded.doc.pages[0], "btn").unwrap();
    assert_eq!(btn.corner_radii, Some([2.0, 8.0, 20.0, 8.0]), "per-corner radii persist");
}
...
fn svg_clipboard_roundtrip_stays_editable() {
    ...
    let frame = Node::frame("clip", 200.0, 100.0)
        .child(Node::rect("r1", 10.0, 10.0, 80.0, 40.0, Color::rgb8(0x22, 0xc5, 0x5e)).radius(6.0));
    let vars = arco_native::Variables::default();
    ...
    assert!(count(&reimported) >= 2, "re-import yields editable node tree");
}
