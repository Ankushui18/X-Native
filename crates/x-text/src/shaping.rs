...
pub fn glyph_outlines(
    ...
    for line in &lines {
        ...
        let baseline =
            y + f0.ascent * (max_size / f0.units_per_em) * style.line_height.clamp(1.0, 1.2);
...
pub fn node_text_outlines_styled(
    ...
    node_text_outlines_styled_uncached(fonts, text, size, max_width, font_name, color, ls, lh, wrap)
}
...
pub fn node_text_outlines_rich(
    ...
    node_text_outlines_rich_uncached(fonts, parts, base_size, max_width, base_font, ls, lh, wrap)
}
...
pub fn node_text_baseline(
    ...
    let chosen = font_name
        .and_then(|n| fonts.font_index(n))
        .or_else(|| fonts.default_font())?;
    let f0 = &fonts.fonts[chosen];
    let max_size = size * 0.72;
    let factor = lh.clamp(1.0, 1.2);
    Some(f0.ascent * (max_size / f0.units_per_em) * factor)
}
    ...
    let spans: Vec<Span> = parts
        ...
        .map(|p| {
            ...
            if let Some(f) = &p.font {
                if let Some(i) = fonts.font_index(f) {
                    sp = sp.font(i);
                }
            }
            sp
