...
pub fn run() {
            ...
            Some((mut d, outcome)) => {
                ...
                match &outcome {
                    ...
                    RecoveredFromAutosave => {
                        recovery_note = Some("recovered unsaved changes from autosave".into())
                    }
                    RecoveredLenient(n) => {
                        recovery_note = Some(format!("recovered corrupt document ({n} note(s))"))
                    }
                    RecoveredFromBackup(b) => {
                        recovery_note = Some(format!("recovered from backup {b}"))
                    }
                }
                let upgraded = x_native::fileio::upgrade_legacy_library_hashes(&mut d);
                if !upgraded.is_empty() {
                    eprintln!("library integrity: upgraded legacy hashes for {upgraded:?}");
                }
                d
            }
            None => demo_document(),
    ...
    let pages = if doc.pages.is_empty() {
        vec![Node::frame("page-1", 1600.0, 1000.0)]
    } else {
        doc.pages
    };
    ...
    let mut app = App {
        ...
        rect_radius: 4.0,
        gradient_stop: 0,
        gradient_editing: false,
        fill_layer_index: 0,
        stroke_layer_index: 0,
        effect_layer_index: 0,
        stroke_advanced_open: false,
        scale_factor: 1.0,
        pending_close: false,
        ...
        status: "ready".into(),
        export_scale: 1.0,
        present_device: DeviceFrame::None,
        created_count: 0,
        win_w: 1280.0,
        win_h: 800.0,
        layer_rows: vec![],
        ...
        fonts: {
            ...
            if n > 0 {
                eprintln!("typography: loaded {n} system font(s)");
            }
            fm
        },
        ...
        thumbs_collapsed: std::fs::read_to_string(".xprefs")
            .map(|t| t.contains("thumbs=collapsed"))
            .unwrap_or(false),
        dbl: false,
    ...
    // load-time integrity sweep (review item): verify every pinned snapshot
    {
        ...
        if !frozen.is_empty() {
            eprintln!("library integrity: frozen {frozen:?}");
        }
    }
    ...
    if let Ok(entries) = std::fs::read_dir("assets") {
        ...
        if !app.assets.is_empty() {
            app.status = format!("loaded {} image asset(s)", app.assets.len());
        }
    }
    // decode any embedded asset:// records carried by the opened document
    {
        ...
        if decoded > 0 {
            eprintln!("assets: decoded {decoded} embedded image(s) from the document store");
        }
    }
    ...
    eprintln!(
        "fonts: {} system families + {} google families",
        app.sysfonts.families.len(),
        gf_count
    );
...
impl Gpu {
    ...
    fn resize(&mut self, w: u32, h: u32) {
        ...
        self.offscreen_view =
            Self::make_offscreen(&self.device, self.config.width, self.config.height);
    }
}
...
impl winit::application::ApplicationHandler for AppHost {
    ...
    fn window_event(
        ...
        match event {
            WindowEvent::CloseRequested => app.confirm_close(),
            WindowEvent::Resized(new_size) => {
                gpu.resize(new_size.width, new_size.height);
                // physical surface, logical layout (Retina-aware)
                app.win_w = gpu.config.width as f64 / app.scale_factor;
                app.win_h = gpu.config.height as f64 / app.scale_factor;
                window.request_redraw();
            }
            // macOS Retina: moving between displays / DPI changes. Keep the
            // LOGICAL size stable and let winit deliver the new physical
            // size via Resized.
            WindowEvent::ScaleFactorChanged {
                ...
                // keep the LOGICAL size stable, re-express in physical px
                let _ = inner_size_writer.request_inner_size(PhysicalSize::new(
                    (app.win_w * scale_factor).round() as u32,
                    (app.win_h * scale_factor).round() as u32,
                ));
                window.request_redraw();
            }
            ...
            WindowEvent::CursorMoved { position, .. } => {
                // physical pixels -> logical (Retina-aware)
                app.mouse_move(Point::new(
                    position.x / app.scale_factor,
                    position.y / app.scale_factor,
                ));
            ...
            WindowEvent::MouseInput {
                ...
                if app.screen == Screen::Dashboard {
                    ...
                    for (tag, r, kind) in app.dash_layout() {
                        if kind == 0 && r.contains(app.cursor) {
                            app.dash_ctx_path = Some(tag);
                            break;
                        }
                    }
                    if let Some(path) = app.dash_ctx_path.clone() {
                ...
                if app.canvas_rect().contains(app.cursor) && app.present.is_none() {
                    ...
                    let hit = x_native::editor::hit_test(&app.editor.root, wp).and_then(|id| {
                        x_native::editor::top_level_ancestor(&app.editor.root, &id).or(Some(id))
                    });
                    let inside_selection = hit
                        .as_ref()
                        .is_some_and(|h| app.editor.selection.contains(h));
                    if !inside_selection {
                    ...
                    app.ctx_menu.items = vec![
                        ...
                        x_native::ui::MenuItem {
                            ...
                            enabled: app.editor.clipboard_len() > 0,
                        },
                        x_native::ui::MenuItem {
                            label: "DUPLICATE".into(),
                        ...
                        x_native::ui::MenuItem {
                            ...
                            enabled: app.editor.selection.len() >= 2,
                        },
                        x_native::ui::MenuItem {
                            label: "FRAME SELECTION".into(),
                            shortcut: Some("⌥⌘G".into()),
                            enabled: has_sel,
                        },
                        x_native::ui::MenuItem {
            ...
            WindowEvent::MouseWheel { delta, .. } => {
                ...
                let (dx, dy) = match delta {
                    ...
                    // trackpad pixel deltas arrive in physical px (Retina-aware)
                    MouseScrollDelta::PixelDelta(p) => {
                        (p.x / app.scale_factor, p.y / app.scale_factor)
                    }
                };
                // present mode: wheel scrolls the scrollable frame under the cursor
                if app.present.is_some() {
                    app.present_scroll(dx, dy);
                    window.request_redraw();
                    return;
                }
                ...
                if app.cursor.x > app.win_w - INSPECTOR_W && app.cursor.y > TOP_H {
                    if dy < 0.0 {
                        app.font_scroll = app.font_scroll.saturating_add(3);
                    } else if dy > 0.0 {
                        app.font_scroll = app.font_scroll.saturating_sub(3);
                    }
                } else if app.cursor.x < LAYERS_W && app.cursor.y > TOP_H {
                    if dy < 0.0 {
                        app.layers_scroll = app.layers_scroll.saturating_add(2);
                    } else if dy > 0.0 {
                        app.layers_scroll = app.layers_scroll.saturating_sub(2);
                    }
                } else if app.ctrl {
            ...
            // trackpad pinch-to-zoom (macOS magnify gesture). `delta` is the
            // incremental scale change (NSEvent.magnification): > 0 = pinch
            // out (zoom in). Available on macOS; no-op elsewhere.
            WindowEvent::PinchGesture { delta, .. } => {
                let factor = (1.0 + delta).clamp(0.9, 1.1);
                let before = app.world_point(app.cursor);
                app.zoom = (app.zoom * factor).clamp(0.05, 16.0);
                let after = app.world_point(app.cursor);
            ...
            WindowEvent::KeyboardInput { event: kev, .. } => {
                ...
                if kev.state == ElementState::Pressed {
                    ...
                    if app.focus != Focus::None {
                        match &kev.logical_key {
                            ...
                            Key::Named(NamedKey::Escape) => {
                                if app.focus == Focus::LayerSearch {
                            ...
                            Key::Named(NamedKey::Backspace) => {
                                match &mut app.focus {
                                    ...
                                    Focus::VariantProp { buffer, .. } => {
                                        buffer.pop();
                                    }
                                    Focus::Proto { buffer, .. } => {
                                        buffer.pop();
                                    }
                                    Focus::PresentVar { buffer, .. } => {
                                        buffer.pop();
                                    }
                                    Focus::LayerSearch => {
                                        app.layer_filter.pop();
                                    }
                                    Focus::LayerRename { buffer, .. } => {
                                        buffer.pop();
                                    }
                                    Focus::FontSearch => {
                                        app.font_query.pop();
                                    ...
                                    Focus::CodeRef { buffer, .. } => {
                                        buffer.pop();
                                    }
                                    Focus::StyleRename { buffer, .. } => {
                                        buffer.pop();
                                    }
                                    Focus::AssetSearch => {
                                        app.asset_query.pop();
                                    }
                                    Focus::AssetRename { buffer, .. } => {
                                        buffer.pop();
                                    }
                                    Focus::PageRename { buffer, .. } => {
                                        buffer.pop();
                                    }
                                    Focus::DashSearch => {
                                        app.dash_query.pop();
                                    }
                                    Focus::DashRename { buffer, .. } => {
                                        buffer.pop();
                                    }
                                    Focus::None => {}
                                }
                            ...
                            Key::Named(NamedKey::ArrowLeft) => {
                                ...
                                {
                                    if app.shift {
                                        if sel_anchor.is_none() {
                                            *sel_anchor = Some(*caret);
                                        }
                                    } else {
                                        *sel_anchor = None;
                                    }
                            ...
                            Key::Named(NamedKey::ArrowRight) => {
                                ...
                                } = &mut app.focus
                                {
                                    ...
                                    if *caret < buffer.len() {
                                        let mut c = *caret + 1;
                                        while c < buffer.len() && !buffer.is_char_boundary(c) {
                                            c += 1;
                                        }
                                        *caret = c;
                                    }
                            ...
                            Key::Named(NamedKey::Space) => match &mut app.focus {
                                Focus::TextNode {
                                    buffer,
                                    caret,
                                    sel_anchor,
                                ...
                                | Focus::AssetRename { buffer, .. }
                                | Focus::PageRename { buffer, .. }
                                | Focus::DashRename { buffer, .. } => buffer.push(' '),
                                Focus::Prop { buffer, .. } => buffer.push(' '),
                                Focus::VariantProp { buffer, .. } => buffer.push(' '),
                                Focus::Proto { buffer, .. } => buffer.push(' '),
                                Focus::PresentVar { buffer, .. } => buffer.push(' '),
                                Focus::DashSearch => app.dash_query.push(' '),
                                Focus::LibSearch => app.lib_query.push(' '),
                                Focus::CodeRef { buffer, .. } => buffer.push(' '),
                                _ => {}
                            },
                            Key::Character(c) => {
                                        ...
                                        match c.as_str().to_ascii_lowercase().as_str() {
                                            ...
                                            "v" => {
                                                if let Some(t) = crate::os_clipboard_get() {
                                                    ...
                                                    *caret += t.len();
                                                    app.status = format!(
                                                        "pasted {} char(s) from OS clipboard",
                                                        t.len()
                                                    );
                                                } else {
                                                    app.status =
                                                        "OS clipboard empty/unavailable".into();
                                                }
                                            }
                                            "b" | "i" => {
                                                // rich text: ⌘B bold / ⌘I italic over the caret
                                                // selection (Figma style-override toggle).
                                                let (lo, hi) = sel_anchor
                                                    .map(|a| (a.min(*caret), a.max(*caret)))
                                                    .unwrap_or((*caret, *caret));
                                                if lo < hi
                                                    && app.editor.toggle_span_style(
                                                        &tid,
                                                        lo,
                                                        hi,
                                                        c.as_str() == "b",
                                                    )
                                                {
                                                    ...
                                                    app.status =
                                                        "select text to apply style".into();
                                                }
                                            }
                                            _ => {}
                                        }
                                ...
                                match &mut app.focus {
                                    ...
                                    Focus::Prop { buffer, .. } => buffer.push_str(c.as_str()),
                                    Focus::VariantProp { buffer, .. } => {
                                        buffer.push_str(c.as_str())
                                    }
                                    Focus::Proto { buffer, .. } => buffer.push_str(c.as_str()),
                                    Focus::PresentVar { buffer, .. } => buffer.push_str(c.as_str()),
                                    Focus::LayerSearch => app.layer_filter.push_str(c.as_str()),
                                    Focus::LayerRename { buffer, .. } => {
                                        buffer.push_str(c.as_str())
                                    }
                                    Focus::FontSearch => {
                                        app.font_query.push_str(c.as_str());
                                        app.refresh_font_results();
                                    }
                                    Focus::StyleSearch => app.style_query.push_str(c.as_str()),
                                    Focus::LibSearch => app.lib_query.push_str(c.as_str()),
                                    Focus::CodeRef { buffer, .. } => buffer.push_str(c.as_str()),
                                    Focus::StyleRename { buffer, .. } => {
                                        buffer.push_str(c.as_str())
                                    }
                    ...
                    match &kev.logical_key {
                        Key::Named(NamedKey::Enter) if app.present.is_some() => {
                            app.present_key_down("Enter");
                        }
                        Key::Named(NamedKey::Space) if app.present.is_some() => {
                            app.present_key_down("Space");
                        }
                        ...
                        Key::Named(NamedKey::Escape) => {
                            if app.menu_open.is_some() {
                                app.menu_open = None;
                            } else if app.pen_target.is_some() {
                                app.pen_target = None;
                                app.pen_placing = None;
                                app.pen_pending_out = None;
                                app.status = "pen: path finished".into();
                            } else if app.node_edit.is_some() {
                                app.node_edit = None;
                                app.status = "node edit: done".into();
                            } else if app.help_open {
                                app.help_open = false;
                            } else if app.present.is_some() {
                                app.present = None;
                                app.status = "exited presentation".into();
                                ...
                                let parent =
                                    x_native::editor::top_level_ancestor(&app.editor.root, &id);
                                match parent {
                                    Some(pid) if pid != id => {
                                        app.editor.selection = vec![pid];
                                        app.status = "selected parent".into();
                                    }
                                    _ => {
                        ...
                        Key::Character(c) => {
                            let ch = c.as_str();
                            // present mode: KeyDown prototype interactions
                            // consume the key before any tool shortcut
                            if app.present.is_some() && app.present_key_down(ch) {
                                window.request_redraw();
                            } else if app.ctrl {
                                ...
                                match lower.as_str() {
                                    "z" => {
                                        ...
                                        app.status = "undo/redo".into();
                                    }
                                    "f" if app.shift => {
                                        app.perf_hud = !app.perf_hud;
                                        app.status = if app.perf_hud {
                                            "perf HUD on".into()
                                        } else {
                                            "perf HUD off".into()
                                        };
                                    ...
                                    "d" => {
                                        app.editor.duplicate_selection((16.0, 16.0));
                                        app.status = "duplicated".into();
                                    }
                                    "c" => app.clipboard_copy(),
                                    "x" => app.clipboard_cut(),
                                    "v" => {
                                        // Figma paste family: V = paste in
                                        // place; Shift+V = paste over
                                        // selection; Alt+Shift+V = replace.
                                        if app.shift && app.alt {
                                            app.clipboard_paste_to_replace();
                                        } else if app.shift {
                                            app.clipboard_paste_over_selection();
                                        } else {
                                            app.clipboard_paste();
                                        }
                                    }
                                    ...
                                    "." => {
                                        ...
                                        app.status = if app.chrome_hidden {
                                            "UI hidden".into()
                                        } else {
                                            "UI shown".into()
                                        };
                                    ...
                                    "y" => {
                                        app.outline_view = !app.outline_view;
                                        app.status = if app.outline_view {
                                            "outline view".into()
                                        } else {
                                            "normal view".into()
                                        };
                                    ...
                                    ";" => {
                                        app.user_guides.clear();
                                        app.status = "guides cleared".into();
                                    }
                                    "0" => {
                                        app.zoom = 1.0;
                                        app.status = "zoom 100%".into();
                                    }
                                    "1" => {
                                        // zoom-to-fit the page in the canvas area
                                        let cw =
                                            app.win_w - TOOLBAR_W - LAYERS_W - INSPECTOR_W - 40.0;
                                        ...
                                        app.pan = (20.0, 20.0);
                                        app.status =
                                            format!("zoom to fit ({:.0}%)", app.zoom * 100.0);
                                    }
                                    "g" => {
                                        if app.alt {
                                            // Figma ⌥⌘G = "Frame selection"
                                            if !app.editor.selection.is_empty() {
                                                let fid =
                                                    format!("frame-{}", app.editor.undo_depth());
                                                app.editor.frame_selection(&fid);
                                                app.status = format!("frame selection -> {fid}");
                                            } else {
                                                app.status = "select 1+ nodes to frame".into();
                                            }
                                        } else if app.shift {
                                            if let Some(id) = app.editor.selection.first().cloned()
                                            {
                                                if app.editor.ungroup(&id) {
                                                    app.status = "ungrouped".into();
                                                }
                                            }
                                    ...
                                    "a" => {
                                        if app.shift {
                                            // ⌘⇧A = "Frame selection" (roadmap shortcut; ⌥⌘G also works)
                                            if !app.editor.selection.is_empty() {
                                                let fid =
                                                    format!("frame-{}", app.editor.undo_depth());
                                                app.editor.frame_selection(&fid);
                                            ...
                                            app.status =
                                                format!("{} selected", app.editor.selection.len());
                                        }
                                    }
                                    "k" => {
                                        ...
                                        let name = format!(
                                            "Component{}",
                                            app.editor.component_names().len() + 1
                                        );
                                        if app.editor.make_component(&name) {
                                            app.status = format!(
                                                "created component {name} from {n} node(s)"
                                            );
                                        } else {
                                    ...
                                    "]" => {
                                        if let Some(id) = app.editor.selection.first().cloned() {
                                            app.editor.bring_to_front(&id);
                                            app.status = "to front".into();
                                        }
                                    }
                                    "[" => {
                                        if let Some(id) = app.editor.selection.first().cloned() {
                                            app.editor.send_to_back(&id);
                                            app.status = "to back".into();
                                        }
                                    }
                                    _ => {}
                                }
                                app.last_cmd = Some((
                                    format!("ctrl+{lower}"),
                                    cmd_t0.elapsed().as_secs_f32() * 1000.0,
                                ));
                            } else {
                                match ch.to_ascii_lowercase().as_str() {
                                    ...
                                    "a" => {
                                        if app.shift {
                                            ...
                                            if app.asset_browser {
                                                app.status =
                                                    "asset browser (Shift+A to close)".into();
                                            } else {
                                                ...
                                                x_native::collect_asset_ids(
                                                    &app.editor.root,
                                                    &mut keep,
                                                );
                                                for (i, pg) in app.pages.iter().enumerate() {
                                                    if i != app.page_idx {
                                                        x_native::collect_asset_ids(pg, &mut keep);
                                                    }
                                                }
                                    ...
                                    "h" if app.shift => {
                                        ...
                                        for id in ids {
                                            app.editor.flip_node(&id, true);
                                        }
                                        app.editor.merge_last(
                                            app.editor.undo_depth().saturating_sub(depth),
                                        );
                                    ...
                                    "e" if app.shift => {
                                        // vector-edit eraser (Figma Draw
                                        // Shift+E): only meaningful while
                                        // a path's anchors are on screen
                                        if app.node_edit.is_some() {
                                            app.vector_eraser = !app.vector_eraser;
                                            app.status = if app.vector_eraser {
                                                "eraser: drag across segments to erase them".into()
                                            } else {
                                                "eraser off".into()
                                            };
                                        } else {
                                            app.status =
                                                "eraser: double-click a path to edit it first"
                                                    .into();
                                        }
                                    }
                                    ...
                                    "r" => {
                                        if app.shift {
                                            app.rulers = !app.rulers;
                                            app.status = if app.rulers {
                                                "rulers on (click ruler to add guide)".into()
                                            } else {
                                                "rulers off".into()
                                            };
                                        } else {
                                    ...
                                    "o" => {
                                        if app.shift {
                                            app.tool = Tool::Arc;
                                            app.status =
                                                "arc: drag to size, edit angles in inspector"
                                                    .into();
                                        } else {
                                    ...
                                    "p" => {
                                        if app.shift {
                                            ...
                                            app.status = "pencil: drag to sketch".into();
                                        } else {
                                            app.tool = Tool::Polygon;
                                        }
                                    }
                                    "s" => app.tool = Tool::Slice,
                                    "t" => app.tool = Tool::Text,
                                    "i" => {
                                        if app.shift {
                                            app.tool = Tool::Bucket;
                                            app.status =
                                                "paint bucket: click a layer to fill it".into();
                                        } else {
                                    ...
                                    "b" => {
                                        if app.shift {
                                            app.tool = Tool::Brush;
                                            app.status =
                                                "brush: drag to paint (speed controls width)"
                                                    .into();
                                        } else {
                                            app.tool = Tool::Pen;
                                            app.pen_target = None;
                                    ...
                                    "]" => {
                                        if let Some(id) = app.editor.selection.first().cloned() {
                                            app.editor.bring_forward(&id);
                                            app.status = "forward".into();
                                        }
                                    }
                                    "[" => {
                                        if let Some(id) = app.editor.selection.first().cloned() {
                                            app.editor.send_backward(&id);
                                            app.status = "backward".into();
                                        }
                                    }
                                    "d" if app.present.is_some() => {
                                        app.present_device = app.present_device.next();
                                        app.status =
                                            format!("device frame: {}", app.present_device.label());
                                    }
                                    _ => {}
                                }
                                // "]"/"[" report their own status (z-order
                                // change), not a tool switch — don't stomp it.
                                if !app.shift && ch != "]" && ch != "[" && app.present.is_none() {
                                    app.status = format!("tool: {:?}", app.tool);
                                }
                            }
                        }
                        _ => {}
            ...
            WindowEvent::RedrawRequested => {
                ...
                if app.editor.undo_depth() != app.saved_undo_depth {
                    app.dirty_since_save = true;
                }
                ...
                // Retina: the scene is built in LOGICAL coords; scale it up
                // to the physical surface (1.0 on standard-DPI displays).
                let render_scene = if app.scale_factor != 1.0 {
                    let mut s = Scene::new();
                    s.append(&scene, Some(Affine::scale(app.scale_factor)));
                    s
                } else {
                    scene
                };
                ...
                let _ = gpu.renderer.render_to_texture(
                    ...
                    &gpu.offscreen_view,
                    &RenderParams {
                        base_color: if app.present.is_some() {
                            Color::BLACK
                        } else {
                            C_CANVAS
                        },
