...
fn main() {
    ...
    vars.colors.insert("bg".into(), Color::from_rgb8(0xff, 0xff, 0xff));
    ...
    let page = Node::frame("page-1", 1000.0, 700.0)
        ...
        .child(
            Node::rect("card", 0.0, 0.0, 240.0, 140.0, Color::from_rgb8(0x0d, 0x99, 0xff))
                .radius(18.0)
                .rotate(PI / 8.0)
        ...
        .child(
            Node::rect("grad", 0.0, 0.0, 200.0, 120.0, Color::WHITE).fill_paint(Paint::LinearGradient {
                ...
                stops: vec![(0.0, Color::from_rgb8(0xff, 0x5a, 0x00)), (1.0, Color::from_rgb8(0x8e, 0x2d, 0xe2))],
            }),
        )
        .child(Node::ellipse("dot", 0.0, 0.0, 120.0, 120.0, Color::from_rgb8(0xf2, 0x48, 0x22)).opacity(0.75).blend(BlendKind::Multiply))
    ...
    ed.undo();
    ed.redo();
    println!("undo/redo:   card.x after move+undo+redo = {}", x_native::editor::find(&ed.root, "card").unwrap().transform.x);
    ...
    println!("group:       group-1 children={}", x_native::editor::find(&ed.root, "group-1").unwrap().children.len());
    ed.undo();
    println!("ungroup(undo): group exists = {}", x_native::editor::find(&ed.root, "group-1").is_some());
    ...
    println!("copy/paste:  pasted ids={:?}, undoable={}", pasted, { ed.undo(); x_native::editor::find(&ed.root, "card-copy").is_none() });
    ...
    let from = Node::frame("s1", 400.0, 400.0).child(Node::rect("box", 0.0, 0.0, 100.0, 100.0, Color::from_rgb8(255, 0, 0)));
    let to = Node::frame("s2", 400.0, 400.0).child(Node::rect("box", 200.0, 100.0, 200.0, 100.0, Color::from_rgb8(0, 0, 255)));
    ...
    println!("smart anim:  t=0.5 -> x={} w={} fill={}", b.transform.x, b.w,
        if let x_native::Paint::Solid(c) = &b.fill { x_native::color_to_hex(*c) } else { "?".into() });

    // ---- dev mode -----------------------------------------------------------
    let css = x_native::editor::node_to_css(x_native::editor::find(&ed.root, "card").unwrap(), &vars);
